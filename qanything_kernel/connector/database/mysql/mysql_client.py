...
from qanything_kernel.configs.model_config import MYSQL_DATABASE, MYSQL_HOST_LOCAL, MYSQL_PASSWORD, MYSQL_PORT, MYSQL_USER
...
class KnowledgeBaseManager:
    ...
    def check_user_exist_(self, user_id):
        ...
        print("check_user_exist {}".format(result))
    ...
    def check_kb_exist(self, user_id, kb_ids):
        ...
        print("check_kb_exist {}".format(result))
    ...
    def check_file_exist(self, user_id, kb_id, file_ids):
        # 筛选出有效的文件
        if not file_ids:
            print("check_file_exist skipped because of empty file_ids")
            return []
        ...
        print("check_file_exist {}".format(result))
        return result
    ...
    def check_file_exist_by_name(self, user_id, kb_id, file_names):
        ...
        for i in range(0, len(file_names), batch_size):
            ...
            print("check_file_exist_by_name batch {}: {}".format(i // batch_size, batch_result))
    ...
    def new_kb_base(self, kb_id, user_id, kb_name, user_name=None):
    ...
    def add_file(self, user_id, kb_id, file_name, timestamp, status="gray"):
        ...
        print("add_file: {}".format(file_id))
        return file_id, "success"


    async def add_file_content(self, user_id, kb_id, local_files):
        # 如果他传回来了一个id, 那就说明这个表里肯定有
        if not self.check_user_exist_(user_id):
            return None, "invalid user_id, please check..."
            ...
            for doc in local_file.docs:
                try:
                    if len(txt) < 500:
                        ...
                        query = "INSERT INTO Content (file_id, content, timestamp) VALUES (%s, %s, %s)"
                        mr = await loop.run_in_executor(
                            self.executor, partial(self.execute_query_, query=query, params=(local_file.file_id, txt, timestamp), commit=True)
                        )
                        txt = ''
                except Exception as e:
                    print("Mysql inser content, error")
                    return False
        return True
    ...
    def delete_files(self, kb_id, file_ids):
        ...
        print("delete_files: {}".format(file_ids))
