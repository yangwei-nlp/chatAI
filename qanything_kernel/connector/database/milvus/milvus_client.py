...
from qanything_kernel.configs.model_config import MILVUS_HOST_LOCAL, MILVUS_PORT, MILVUS_USER, MILVUS_PASSWORD, MILVUS_DB_NAME, CHUNK_SIZE, VECTOR_SEARCH_TOP_K
...
class MilvusClient:
    ...
    def init(self):
        try:
            ...
            if utility.has_collection(self.user_id):
                ...
                print(f'collection {self.user_id} exists')
            else:
                schema = CollectionSchema(self.fields)
                print(f'create collection {self.user_id} {schema}')
            ...
            print('partitions: %s', self.kb_ids)
            self.sess.load()
    ...
    async def insert_files(self, file_id, file_name, file_path, docs, embs, batch_size=1000):
        print(f'now inser_file {file_name}')
        now = datetime.now()
        ...
        for batch_start in range(0, num_docs, batch_size):
            ...
            try:
                ...
                print(f'{file_name} {mr}')
            except Exception as e:
                print(f'Milvus insert file_id:{file_id}, file_name:{file_name} failed: {e}')
                return False
    ...
    def delete_files(self, files_id):
        ...
        print('milvus delete files_id: %s', files_id)
    ...
    def expand_cand_docs(self, cand_docs):
        ...
        print('milvus group number: %s', len(m_grouped))
