...
import time
from qanything_kernel.connector.llm import MyLLM
...
Embeddings.__hash__ = _embeddings_hash
...
class LocalDocQA:
    ...
    def init_cfg(self, mode='local'):
        ...
        self.llm: MyLLM = MyLLM()
        self.kb_manager = KnowledgeBaseManager('local')
    ...
    async def insert_files_to_milvus(self, user_id, kb_id, local_files: List[LocalFile]):
        print(f'insert_files_to_milvus: {kb_id}')
        ...
        for local_file in local_files:
            ...
            print(f'split time: {end - start} {len(local_file.docs)}')
            start = time.time()
            try:
            ...
            print(f'embedding time: {end - start} {len(local_file.embs)}')
            ...
            print(f'insert time: {insert_time - end}')
            if ret:
    ...
    def get_source_documents(self, queries, milvus_kb, cosine_thresh=None, top_k=None):
        ...
        t2 = time.time()
        print(f"milvus search time: {t2 - t1}")
    ...
    def reprocess_source_documents(self, query: str,
        ...
        print(f"limited token nums: {limited_token_nums}")
        print(f"template token nums: {template_token_num}")
        print(f"query token nums: {query_token_num}")
        print(f"history token nums: {history_token_num}")
        print(f"new_source_docs token nums: {self.llm.num_tokens_from_docs(new_source_docs)}")
    ...
    def rerank_documents_for_local(self, query, source_documents):
        ...
        try:
            ...
            print("rerank error: %s", traceback.format_exc())
            print("rerank error, use origin retrieval docs")
