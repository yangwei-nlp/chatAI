...
__all__ = ["new_knowledge_base", "upload_files", "list_kbs", "list_docs", "delete_knowledge_base", "delete_docs",
           "rename_knowledge_base", "get_total_status", "clean_files_by_status", "upload_weblink", "local_doc_chat", "doc_summary",
           "document"]
...
async def upload_files(req: request):
    ...
    for file, file_name in zip(files, file_names):
        ...
        print(f"{file_name}, {file_id}, {msg}")
        local_file = LocalFile(user_id, kb_id, file, file_id, file_name, local_doc_qa.embeddings)
        local_files.append(local_file)
        local_doc_qa.kb_manager.update_file_size(file_id, len(local_file.file_content))
        data.append(
    ...
    # 文件OCR文本存入mysql
    asyncio.create_task(local_doc_qa.kb_manager.add_file_content(user_id, kb_id, local_files))
    if exist_file_names:
...
async def get_total_status(req: request):
    ...
    if not user_id:
        users = local_doc_qa.kb_manager.get_users()
    ...
    for user in users:
        res[user] = {}
        kbs = local_doc_qa.kb_manager.get_knowledge_bases(user)
        for kb_id, kb_name in kbs:
            gray_file_infos = local_doc_qa.kb_manager.get_file_by_status([kb_id], 'gray')
            red_file_infos = local_doc_qa.kb_manager.get_file_by_status([kb_id], 'red')
            yellow_file_infos = local_doc_qa.kb_manager.get_file_by_status([kb_id], 'yellow')
            green_file_infos = local_doc_qa.kb_manager.get_file_by_status([kb_id], 'green')
...
async def doc_summary(req: request):
    """总结函数"""
    local_doc_qa: LocalDocQA = req.app.ctx.local_doc_qa
    user_id = safe_get(req, 'user_id')
    if user_id is None:
