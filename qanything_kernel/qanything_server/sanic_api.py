...
# 获取`qanything_kernel`目录的父目录，即`qanything_kernel`
...
async def init_local_doc_qa(app, loop):
    ...
    local_doc_qa.init_cfg('local')
    print(f'init local_doc_qa in local')
...
# app.add_route(doc_summary, "/api/local_doc_qa/local_doc_chat", methods=['POST'])  # tags=["问答接口"]
